...
export default function SubscribeForm({ compact = false }) {
  ...
  const inputBase = {
    ...
    borderRadius: 12,
    border: status === 'error' ? '1px solid #ef4444' : '1px solid var(--border-color, #e5e7eb)',
    outline: 'none',
  ...
  const formInner = (
    <form onSubmit={onSubmit} style={{ display: 'flex', gap: 10, flexWrap: 'wrap', alignItems: 'center' }}>
      <div style={{ position: 'relative', display: 'flex', alignItems: 'center' }}>
        <svg width="16" height="16" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" aria-hidden="true" style={{ position: 'absolute', left: 10, color: 'var(--text-secondary, #6b7280)' }}>
